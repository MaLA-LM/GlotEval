...
## 1. Motivation
GlotEval targets massively multilingual MT evaluation, so we need to show it produces trustworthy scores that agree with widely used toolkits. The goal of this study is to audit GlotEval against the LM Evaluation Harness (LM-Eval) on the FLORES-200 benchmark and verify that both frameworks yield comparable ChrF scores under matched settings. A collaborating teammate Tianxiang Wang @congc100 independently ran the LM-Eval pipeline, providing reference outputs used in this comparison.
...
- **Visual confirmation**: The ChrF-only combined plots (`consistency_plots/*.png`) highlight near-diagonal clustering for every model/setting except the noted outlier.
...
_Acknowledgement: Many thanks to our collaborator Tianxiang Wang @congc100 for running the LM Evaluation Harness reference experiments that enabled this study._
